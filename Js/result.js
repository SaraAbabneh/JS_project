...
[  
  ...
  },
  {
    ...
    "Answer": [{ "Answer": "constant int x = 5", "value": 1 }, { "Answer": "final int x = 5", "value": 0 },{ "Answer": "const int x = 5", "value": 0 }, { "Answer": "int final x = 5", "value": 0 }]
  },
  {
    "id": 18,
...
{"id": 20,
"Question":"What CSS property is used to add space between the elements' content and its border?",
"Answer":[
...
{
  Mark:10,
  answer:["24","35","5","47","48 mph","am","too","He doesn't like coffee.","Limited","Limited","Cascading Style Sheets","background-color","array","Hyperlink and Text Markup Language","rounded-corners","Set a 10px margin on all sides.","constant int x = 5","src","font-size","padding",]
};

let user_answer_lenght=(user_answer.answer).length;
// console.log(user_answer_lenght);

 document.getElementById("Marks Scored").innerHTML=`Marks Scored : ${user_answer.Mark}`
          ...
          ele.Answer.forEach((answer, answerIndex) => {
              ...
              if (answer.value === 1) {
                correctAnswer.push(answer.Answer);
                  ...
                  htmlContent += `
                      <li class="mb-2 correct-answer">${letterLabel}. ${answer.Answer}</li>
                  `;
                  // fauls from JSON
              } else {
                  htmlContent += `
                      <li class="mb-2 incorrect-answer">${letterLabel}. ${answer.Answer}</li> 
                  `;
              }
          });
...
function check_answer(){
  ...
  for (let j = 0; j < correctAnswer.length; j++) {
    ...
    li.forEach( function(ele){
      if (ele.innerHTML.includes(user_answer.answer[j]))
...
// function show_user_answer(user_answer) {
//   let correctAnswers = 0;

//   user_answer.answer.forEach((userAnswer, index) => {
//       const questionSet = answer[0];
//       const question = questionSet[index];

//       // Convert userAnswer to uppercase before finding the corresponding answer
//       const userAnswerUpperCase = userAnswer.toUpperCase();
//       if (question.Answer[userAnswerUpperCase.charCodeAt(0) - 65].value === 1) {
//           correctAnswers++;
//       }
//   });

//   user_answer.Mark = correctAnswers;
//   document.getElementById("Marks_Scored").innerHTML = `Marks Scored: ${user_answer.Mark}`;
// }

// show_user_answer(user_answer);





// function showa_usernswer(user_answer,letterLabel,answer_Answer) {
  
//   let letterLabel_answer_Answer=L_a_A;
//   for (const ele of user_answer.answer) {
//     console.log(ele)

//     if (ele !== L_a_A){
//       document.getElementsByClassName("correct-answer").styel =
